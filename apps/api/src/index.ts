...
import { config, isLocalDevelopmentOrigin, isNativeAppOrigin } from './config.js';
...
app.setErrorHandler((unknownError, request, reply) => {
  ...
  const status = typeof statusCode === 'number' ? statusCode : invalidRequest ? 400 : 500;
  ...
  const message = invalidRequest
    ...
    : status >= 500 && !exposeMessage ? 'No fue posible completar la solicitud.' : error.message;

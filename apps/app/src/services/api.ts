...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    const error = new Error(data.message ?? `Error ${response.status}`) as Error & { code?: string; status?: number };
...
export async function resendRegistrationOtp(email: string): Promise<{
  ...
  return request('/v1/auth/resend-registration-code', {
    method: 'POST',
    body: JSON.stringify({ email }),
  });
}
export async function requestPasswordReset(email: string): Promise<{ sent: true; verificationType: 'recovery'; delivery: 'otp' }> {
  return request('/v1/auth/request-password-reset', {
    method: 'POST',
    body: JSON.stringify({ email }),
  });
}
export async function requestAuthOtp(input: { email: string; fullName: string; preferredName: string }): Promise<{
  sent: true;
  verificationType: OtpVerificationType;
  delivery: OtpDelivery;
}> {
  return request('/v1/auth/request-otp', {
    method: 'POST',
    body: JSON.stringify(input),
  });
}
...
export interface VoiceCreditOffer {
  id: 'voice_5' | 'voice_10' | 'voice_30' | 'voice_60';
  minutes: number;
  seconds: number;
  amount: number;
  currency: string;
...
export function reconcileCheckout(sessionId: string): Promise<{
  ...
  return request('/v1/billing/reconcile-checkout', {
    method: 'POST',
...
export async function createCheckout(interval: 'month' | 'year', returnUrl: string): Promise<{ url: string; trialGranted: boolean }> {
  return request('/v1/billing/checkout', {
    method: 'POST',
    body: JSON.stringify({ interval, returnUrl }),
  });
}
...
export function verifyApplePurchase(input: { transactionId: string; jwsRepresentation: string }): Promise<{ entitlement: BillingEntitlement }> {
  return request('/v1/billing/apple/verify', {
    method: 'POST',
    body: JSON.stringify(input),
  });
}

export function createVoiceCheckout(packageId: VoiceCreditOffer['id'], returnUrl: string): Promise<{ url: string }> {
  return request('/v1/billing/voice/checkout', {
    method: 'POST',
    body: JSON.stringify({ packageId, returnUrl }),
  });
}

export function reconcileVoiceCheckout(sessionId: string): Promise<{
  reconciled: boolean;
  pending: boolean;
  voiceBalance: VoiceCreditBalance;
}> {
  return request('/v1/billing/voice/reconcile-checkout', {
    method: 'POST',
    body: JSON.stringify({ sessionId }),
  });
}

export function verifyAppleVoicePurchase(input: { transactionId: string; jwsRepresentation: string }): Promise<{ voiceBalance: VoiceCreditBalance }> {
  return request('/v1/billing/voice/apple/verify', {
    method: 'POST',
    body: JSON.stringify(input),
  });
}
...
export interface PhotoAnalysis {
  items: Array<{
    ...
    fatG: number;
    confidence: number;
    dataSource: 'vision' | 'vitamate' | 'usda' | 'open_food_facts';
...
export interface CoachChatContext {
  ...
  sleepSummary?: {
    ...
    averageMinutes7Days?: number;
    recent: Array<Pick<SleepEntry, 'startedAt' | 'endedAt' | 'durationMinutes' | 'quality' | 'source'>>;
  };
  mealPlanContext?: string;
  planChangeTarget?: {
    type: 'replace_meal' | 'replace_ingredient';
    slotId?: string;
    ingredient?: string;
  };
}
...
export type CoachAction =
  ...
  | {
      ...
      change: { slotId: string; option: MealPlanOption };
    }
  | {
  ...
  return request<CoachReply>('/v1/coach/chat', {
    ...
    body: JSON.stringify({
      ...
      clientMessage: clientMessage
        ...
        : undefined,
      memory: memory.slice(0, 12).map(({ key, category, content, importance, confidence, expiresAt, updatedAt }) => ({
        key,
        category,
        content,
        importance,
...
export function startCoachCall(callSessionId: string): Promise<{ started: true }> {
  return request('/v1/coach/calls/start', {
    method: 'POST',
    body: JSON.stringify({ callSessionId }),
  });
}

export function heartbeatCoachCall(callSessionId: string): Promise<{ active: boolean }> {
  return request('/v1/coach/calls/heartbeat', {
    method: 'POST',
    body: JSON.stringify({ callSessionId }),
  });
}

export async function recordCoachCall(input: { callSessionId: string; locale: AppLocale; durationSeconds: number; startedAt: string; endedAt: string; usage?: RealtimeCallUsage }): Promise<{ voiceBalance: VoiceCreditBalance }> {
  return request<{ recorded: boolean; voiceBalance: VoiceCreditBalance }>('/v1/coach/calls', {
    method: 'POST',
    body: JSON.stringify(input),
  });
}

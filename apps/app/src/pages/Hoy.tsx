...
import { barbellOutline, chatbubbleEllipsesOutline, chevronForward, closeOutline, restaurantOutline } from 'ionicons/icons';
...
const Hoy = ({ snapshot, isPremium, onRequestPremium }: { snapshot: VitamateSnapshot; isPremium: boolean; onRequestPremium(): void }) => {
  ...
  return <IonPage className="app-page"><IonContent fullscreen>
    <main className="page-shell dashboard-shell">
      ...
      <div className="dashboard-grid">
        ...
        {isPremium ? <section className="week-card"><p className="eyebrow">Entrenamiento semanal</p><strong>{sessions.length}<small> / {targetSessions} sesiones</small></strong><div className="energy-track"><span style={{ width: `${percentage(weeklyWorkout.minutes, weeklyWorkout.targetMinutes)}%` }} /></div><p>{weeklyWorkout.remainingMinutes ? `Llevas ${weeklyWorkout.minutes} min. Faltan ${weeklyWorkout.remainingMinutes} min y ~${weeklyWorkout.remainingCalories} kcal de actividad esta semana.` : 'Cumpliste el volumen semanal. El movimiento extra es opcional y debe favorecer tu recuperación.'}</p><IonRouterLink routerLink="/entrenar">Ver plan e historial <IonIcon icon={chevronForward} /></IonRouterLink></section> : <button className="premium-preview-card" onClick={onRequestPremium}><span>Premium</span><strong>Convierte tus registros en un plan completo</strong><small>Entrenamiento, nutrición y VITACOACH se adaptan contigo.</small><IonIcon icon={chevronForward} /></button>}
      </div>
      <section className="quick-section"><h2>Acciones rápidas</h2><div className="quick-grid"><Quick href="/nutricion" icon={restaurantOutline} label="Registrar alimento" />{isPremium && <><Quick href="/entrenar" icon={barbellOutline} label="Iniciar entrenamiento" /><Quick href="/coach" icon={chatbubbleEllipsesOutline} label="Hablar con VITACOACH" accent /></>}</div></section>
      {isPremium && snapshot.workoutSessions.length > 0 && <section className="today-history"><header><h2>Entrenamientos recientes</h2><IonRouterLink routerLink="/entrenar">Ver todos</IonRouterLink></header>{snapshot.workoutSessions.slice(0, 3).map((session) => <article key={session.id}><IonIcon icon={barbellOutline} /><span><strong>{session.workoutTitle}</strong><small>{new Intl.DateTimeFormat('es-MX', { dateStyle: 'medium' }).format(new Date(session.completedAt))}</small></span><b>{session.durationMinutes} min{session.caloriesBurned ? ` · ${session.caloriesBurned} kcal` : ''}</b></article>)}</section>}
    </main>

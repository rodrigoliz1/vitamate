...
import { cardOutline, chevronForward, closeOutline, cloudDoneOutline, mailOutline, personCircleOutline, settingsOutline, shieldCheckmarkOutline, trashOutline, warningOutline } from 'ionicons/icons';
...
const Cuenta = ({ snapshot, cloudEmail, entitlement, onOpenSubscription, onDeleteAccount }: Props) => {
  ...
  return <IonPage className="app-page"><IonContent fullscreen><main className="page-shell account-shell">
    ...
    <button className="account-delete-link" type="button" onClick={() => setDeleteOpen(true)}><IonIcon icon={trashOutline} />Eliminar mi cuenta y mis datos</button>
  </main></IonContent><IonModal isOpen={deleteOpen} onDidDismiss={() => { setDeleteOpen(false); setConfirmation(''); setDeleteError(''); }} className="account-delete-modal"><section className="account-delete-dialog"><button className="subscription-close" aria-label="Cerrar" onClick={() => setDeleteOpen(false)}><IonIcon icon={closeOutline} /></button><span><IonIcon icon={warningOutline} /></span><p className="eyebrow">Acción permanente</p><h2>Eliminar cuenta VITAMATE</h2><p>Se eliminarán tu perfil, comidas, entrenamientos, documentos, memoria y conversaciones. Esta acción no se puede deshacer.</p>{premium && entitlement?.source === 'apple' && <p><strong>Importante:</strong> una suscripción de App Store seguirá cobrándose hasta que la canceles con Apple. Antes de eliminar tu cuenta, abre Suscripción y elige Administrar en App Store.</p>}<label className="field"><span>Escribe ELIMINAR para confirmar</span><input value={confirmation} autoCapitalize="characters" onChange={(event) => setConfirmation(event.target.value.toLocaleUpperCase('es-MX'))} /></label>{deleteError && <p className="form-error">{deleteError}</p>}<IonButton expand="block" color="danger" disabled={deleteBusy || confirmation !== 'ELIMINAR'} onClick={async () => { setDeleteBusy(true); setDeleteError(''); try { await onDeleteAccount(); setDeleteOpen(false); } catch (error) { setDeleteError(error instanceof Error ? error.message : 'No fue posible eliminar la cuenta.'); } finally { setDeleteBusy(false); } }}>{deleteBusy ? <IonSpinner /> : 'Eliminar definitivamente'}</IonButton></section></IonModal></IonPage>;
};

...
import { analyticsOutline, closeOutline, cloudDoneOutline, cloudOfflineOutline, heartOutline, logOutOutline, mailOutline, moonOutline, refreshOutline, scaleOutline, sunnyOutline, syncOutline, trashOutline, trendingDownOutline, trendingUpOutline, walkOutline } from 'ionicons/icons';
import { sessionsThisWeek, type SleepEntry, type UserProfile } from '@vitamate/domain';
...
interface Props {
  ...
  onAddSleep(entry: Omit<SleepEntry, 'id' | 'createdAt'>): string;
...
const Progreso = ({ snapshot, onAddWeight, onAddSleep, onDeleteSleep, onUpdateProfile, theme, onSetTheme, health, cloud, onRequestMagicLink, onSyncCloud, onSignOutCloud }: Props) => {
  const [weight, setWeight] = useState('');
  const [email, setEmail] = useState('');
  const [message, setMessage] = useState('');
  const [editingProfile, setEditingProfile] = useState(false);
  const [sleepOpen, setSleepOpen] = useState(false);
  const [sleepStart, setSleepStart] = useState(() => dateTimeInputValue(new Date(Date.now() - 8 * 3_600_000)));
  const [sleepEnd, setSleepEnd] = useState(() => dateTimeInputValue(new Date()));
  ...
  const averageSleep = recentSleep.length ? Math.round(recentSleep.reduce((sum, entry) => sum + entry.durationMinutes, 0) / recentSleep.length) : 0;
  ...
  const submitSleep = (event: FormEvent) => {
    event.preventDefault();
    const startedAt = new Date(sleepStart);
    const endedAt = new Date(sleepEnd);
    const durationMinutes = Math.round((endedAt.getTime() - startedAt.getTime()) / 60_000);
    if (!Number.isFinite(durationMinutes) || durationMinutes < 30 || durationMinutes > 1_440) return setMessage('Revisa las horas: el descanso debe durar entre 30 minutos y 24 horas.');
    onAddSleep({ startedAt: startedAt.toISOString(), endedAt: endedAt.toISOString(), durationMinutes, quality: sleepQuality as SleepEntry['quality'], source: 'manual' });
    setSleepOpen(false); setMessage('Sueño registrado. VITACOACH ya puede tomarlo en cuenta.');
  };
  ...
  return <IonPage className="app-page"><IonContent fullscreen><main className="page-shell">
    ...
    <section className="sleep-card"><header><span><IonIcon icon={moonOutline} /></span><div><p className="eyebrow">Sueño y recuperación</p><h2>{recentSleep.length ? `${Math.floor(averageSleep / 60)} h ${averageSleep % 60} min en promedio` : 'Empieza a monitorear tu descanso'}</h2><p>Regístralo manualmente, impórtalo de Apple Health o cuéntaselo a VITACOACH.</p></div><IonButton fill="outline" onClick={() => setSleepOpen(true)}>Registrar sueño</IonButton></header>{recentSleep.length > 0 && <div className="sleep-week" aria-label="Sueño reciente">{recentSleep.map((entry) => <article key={entry.id}><div className="sleep-bar"><i style={{ height: `${Math.max(18, Math.min(100, entry.durationMinutes / 540 * 100))}%` }} /></div><strong>{Math.floor(entry.durationMinutes / 60)}h {entry.durationMinutes % 60}m</strong><small>{new Intl.DateTimeFormat('es-MX', { weekday: 'short' }).format(new Date(entry.endedAt))}</small><span>{entry.source === 'apple_health' ? 'Salud' : entry.source === 'vitacoach' ? 'VC' : 'Manual'}</span><button type="button" aria-label="Eliminar registro de sueño" onClick={() => onDeleteSleep(entry.id)}><IonIcon icon={trashOutline} /></button></article>)}</div>}</section>
    ...
    {health.supported && <section className="healthkit-card"><header><span><IonIcon icon={heartOutline} /></span><div><p className="eyebrow">Apple Health</p><h2>{health.summary ? 'Tu actividad y descanso, con más contexto' : 'Conecta la app Salud del iPhone'}</h2></div></header>{health.summary ? <div className="healthkit-stats"><article><IonIcon icon={walkOutline} /><small>Pasos hoy</small><strong>{health.summary.stepsToday?.toLocaleString('es-MX') ?? '—'}</strong></article><article><IonIcon icon={analyticsOutline} /><small>Calorías activas</small><strong>{health.summary.activeCaloriesToday ?? '—'} <b>kcal</b></strong></article><article><IonIcon icon={heartOutline} /><small>Frecuencia en reposo</small><strong>{health.summary.restingHeartRate ?? '—'} <b>lpm</b></strong></article><article><IonIcon icon={moonOutline} /><small>Último sueño</small><strong>{health.summary.sleepEntries[0] ? `${Math.floor(health.summary.sleepEntries[0].durationMinutes / 60)}h ${health.summary.sleepEntries[0].durationMinutes % 60}m` : '—'}</strong></article></div> : <p>VITAMATE solicita permisos separados para pasos, energía activa, frecuencia en reposo y sueño. Si omites uno, los demás siguen funcionando.</p>}<IonButton fill={health.summary ? 'outline' : 'solid'} className={health.summary ? undefined : 'primary-button'} disabled={health.busy} onClick={() => runCloud(health.summary ? health.refresh : health.connect)}>{health.busy ? <IonSpinner /> : <><IonIcon slot="start" icon={health.summary ? syncOutline : heartOutline} />{health.summary ? 'Actualizar desde Salud' : 'Conectar Apple Health'}</>}</IonButton>{health.message && <p className="cloud-message">{health.message}</p>}</section>}
  ...
  </main></IonContent><IonModal isOpen={sleepOpen} onDidDismiss={() => setSleepOpen(false)} className="sleep-modal"><form className="modal-card sleep-form" onSubmit={submitSleep}><header><div><p className="eyebrow">Registro manual</p><h2>¿Cómo dormiste?</h2></div><button type="button" className="icon-button" aria-label="Cerrar" onClick={() => setSleepOpen(false)}><IonIcon icon={closeOutline} /></button></header><label className="field"><span>Hora de dormir</span><input type="datetime-local" value={sleepStart} onChange={(event) => setSleepStart(event.target.value)} required /></label><label className="field"><span>Hora de despertar</span><input type="datetime-local" value={sleepEnd} onChange={(event) => setSleepEnd(event.target.value)} required /></label><label className="field"><span>Calidad percibida · {sleepQuality}/5</span><input type="range" min="1" max="5" step="1" value={sleepQuality} onChange={(event) => setSleepQuality(Number(event.target.value))} /></label><IonButton type="submit" expand="block" className="primary-button">Guardar descanso</IonButton></form></IonModal><IonModal isOpen={editingProfile} onDidDismiss={() => setEditingProfile(false)}>{snapshot.profile && <ProfileEditor profile={snapshot.profile} onSave={(profile) => { onUpdateProfile(profile); setMessage('Perfil actualizado y planes recalculados.'); }} onClose={() => setEditingProfile(false)} />}</IonModal><IonToast isOpen={Boolean(message)} message={message} duration={3000} onDidDismiss={() => setMessage('')} /></IonPage>;
};

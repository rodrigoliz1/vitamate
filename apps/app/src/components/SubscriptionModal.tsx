...
import { createBillingPortal, createCheckout, type BillingEntitlement, type BillingOffer } from '../services/api';
...
export function SubscriptionModal({ isOpen, onDismiss, entitlement, offers, onLeavingForCheckout }: Props) {
  ...
  const start = async (interval: 'month' | 'year') => {
    ...
    try { const { url } = await createCheckout(interval, window.location.origin); onLeavingForCheckout?.(); window.location.assign(url); }
  ...
  const portal = async () => {
    ...
    try { window.location.assign((await createBillingPortal(window.location.origin)).url); }
  ...
  return <IonModal isOpen={isOpen} onDidDismiss={onDismiss} className="subscription-modal"><div className="subscription-shell">
    ...
    {premium ? <div className="current-plan-panel"><strong>Plan Premium {entitlement.billingInterval === 'year' ? 'anual' : 'mensual'}</strong><span>{entitlement.cancelAtPeriodEnd ? 'Termina' : 'Próxima renovación'}: {formatDate(entitlement.currentPeriodEnd)}</span><IonButton expand="block" className="primary-button" disabled={busy !== null} onClick={portal}>{busy === 'portal' ? <IonSpinner /> : 'Administrar en Stripe'}</IonButton></div> : <div className="subscription-options">
      ...
      <p><IonIcon icon={checkmarkCircle} /> {entitlement?.trialUsed ? 'La prueba gratuita de esta cuenta ya fue utilizada.' : '7 días gratis una sola vez. Después se cobra el periodo elegido; cancela desde el portal.'}</p>
    </div>}
...
function PlanButton({ interval, label, badge, offer, busy, onClick }: { interval: 'month' | 'year'; label: string; badge?: string; offer?: BillingOffer; busy: string | null; onClick(interval: 'month' | 'year'): void }) {
  return <button className={interval === 'year' ? 'subscription-plan is-featured' : 'subscription-plan'} disabled={busy !== null} onClick={() => onClick(interval)}>{badge && <b>{badge}</b>}<span><strong>{label}</strong><small>{interval === 'year' ? '12 meses de acompañamiento' : 'Flexibilidad mes a mes'}</small></span><em>{offer ? money(offer.amount, offer.currency) : 'Ver precio'}<small>/{interval === 'year' ? 'año' : 'mes'}</small></em>{busy === interval && <IonSpinner />}</button>;
}
...
function subscriptionStatus(entitlement: BillingEntitlement) { return entitlement.status === 'trialing' ? `Tu prueba termina el ${formatDate(entitlement.trialEnd)}.` : 'Tu acceso está activo y se actualiza mediante Stripe.'; }
